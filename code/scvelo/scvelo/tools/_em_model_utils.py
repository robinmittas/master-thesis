...
def assign_tau(
    ...
    if assignment_mode in {"full_projection", "partial_projection"} or (
        ...
        xt = SplicingDynamics(
            transcription_rate=alpha, splicing_rate=beta, degradation_rate=gamma
        ).get_solution(tpoints)
        xt_ = SplicingDynamics(
            transcription_rate=0,
            splicing_rate=beta,
            degradation_rate=gamma,
            initial_state=[u0_, s0_],
...
def compute_divergence(
    ...
    **kwargs,
    ...
    if u0_ is None or s0_ is None:
        u0_, s0_ = SplicingDynamics(
            transcription_rate=alpha, splicing_rate=beta, degradation_rate=gamma
        ).get_solution(t_, stacked=False)
