...
atac_sc_glue = sc.read(SN_SC_PROCESSED_DIR / "atac_sc_rna_scglue.h5ad")
sn_sc_glue = sc.read(SN_SC_PROCESSED_DIR / "sn_sc_rna_scglue.h5ad")
sn_sc_scvi = sc.read(SN_SC_PROCESSED_DIR / "sn_sc_rna_scvi.h5ad")
...
# %%
plot_subpart_scib(plot_df, other_cols_bio, column_definitions_bio, "bio_scvi.svg")

# %%
plot_subpart_scib(plot_df, other_cols_batch, column_definitions_batch, "batch_scvi.svg")

# %%
plot_subpart_scib(plot_df, score_cols, column_definitions_agg, "aggregate.svg")

# %%
with mplscience.style_context():
    path = FIG_DIR / "benchmark_metrics_min_max_scaled"
    path.mkdir(parents=True, exist_ok=True)
    bm.plot_results_table(save_dir=path)

# %%
with mplscience.style_context():
    path = FIG_DIR / "benchmark_metrics"
    path.mkdir(parents=True, exist_ok=True)
